...
if __name__ == '__main__':
    ...
    for slug in tqdm(slugs, unit='collection'):
        ...
        for x in driver.find_elements(By.XPATH, "//div[contains(text(),'All')][contains(@class,'time-span')]")[:2]:
        ...
        while True:
            try:
                explicitly_wait(driver,By.XPATH,"//div[contains(@class,'price-graph')]//div[contains(@class,'legends_legend')]")
                time.sleep(3)
                break
            except:
                pass
        ...
        for x in driver.find_elements(By.XPATH, "//div[contains(@class,'time-span')]"):
            x.click()
            driver.find_elements(By.XPATH, "//span[contains(text(),'All')]")[-1].click()
            time.sleep(.1)
        time.sleep(1)
        driver.find_element(By.XPATH, "//div[text()='NFTs'][contains(@class,'graph')]").click()

        time.sleep(2)
        a, b, c, d, e, f, g = extract_logs(driver, ["marketcap-volume.*&range=all", "price.*&range=all",
                                                    "holders-chart.*&range=all",
                                                    "concentration-chart-V2.*Value&range=all", "holdingTimeDis",
                                                    "holdingAmountDis", "concentration-chart-V2.*Num&range=all"])
        if a:
            ...
            print_xlsx(
                pd.DataFrame(
                    [overview_timestamp, [collection_name for _ in range(0, len(overview_timestamp))],
                     ...
                     marketcap_volume_data['marketCap']['values']['y'], marketcap_volume_data['volume']['values']['y'],
                     price_data['floorPrice']['values']['y'], price_data['avgPrice']['values']['y']]).T,
                f"Top 100/{rank}_{collection_name}/OverView",
                ['Date', 'Project Title', 'Total Supply', 'Market Cap', 'Volume', 'Floor Price', 'Average Price'],
                [12, 20, 20, 20, 20, 20, 20, 20]
            )
        except:
            print(f"{rank}_{collection_name} 1")
            overview_timestamp = [str(datetime.datetime.fromtimestamp(x / 1000).date()) for x in
                                  marketcap_volume_data['marketCap']['values']['x']]
            print_xlsx(
                pd.DataFrame(
        ...
        try:
            sales_scatter_timestamp = [str(datetime.datetime.fromtimestamp(x / 1000) - datetime.timedelta(hours=9)) for
                                       x in price_data['salesScatter']['values']['x']]

            print_xlsx(
                pd.DataFrame([sales_scatter_timestamp, price_data['salesScatter']['values']['y']]).T,
                f"Top 100/{rank}_{collection_name}/Sales",
                ['Datetime', 'Sales (ETH)'],
                [20, 20]
            )
        except:
            print(f"{rank}_{collection_name} 2")
            pass
        try:
            ...
            print_xlsx(
                pd.DataFrame([holder_chart_timestamp, holders_chart_data['holderChart']['values']['y'],
                              [str(x) if x is int() else 0 for x in
                               holders_chart_data['whaleChart']['values']['y']]]).T,
                f"Top 100/{rank}_{collection_name}/Holder Trends",
                ['Date', 'Holders', 'Whales'],
                [12, 20, 20]
            )
        except:
            print(f"{rank}_{collection_name} 3")
            pass

        try:
            concentration_timestamp = [str(datetime.datetime.fromtimestamp(x / 1000).date()) for x in
                                       concentration_nft_data['allChart']['values']['x']]

            print_xlsx(
                pd.DataFrame([concentration_timestamp,
                              [round(x, 2) for x in concentration_value_data['allChart']['values']['y']],
                              [x if x is int() else 0 for x in concentration_value_data['giantChart']['values']['y']],
                              concentration_nft_data['allChart']['values']['y'],
                              [x if x is int() else 0 for x in concentration_nft_data['giantChart']['values']['y']]]).T,
                f"Top 100/{rank}_{collection_name}/Holding Concentration",
                ['Date', '[Value] All Holders', '[Value] Top 100', '[NFT] All Holders', '[NFT] Top 100'],
                [12, 20, 20, 20, 20]
            )
        except:
            print(f"{rank}_{collection_name} 4")
            pass
        try:
            print_xlsx(
            ...
            print(f"{rank}_{collection_name} 5")
            pass
        driver.quit()
